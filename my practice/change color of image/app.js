...
for(let i=0;i<canvas.width*canvas.height*4;i+=4){
  ...
  datas[i+2]=10;
        ...
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
           let imageinfo = imageData.data;
           for(let i=0;i<imageData.data.length;i+=4){
            if(imageData.data[i]>=8 && imageData.data[i]<=90){
              if(imageData.data[i+1]>=39 && imageData.data[i+1]<=129){
                if(imageData.data[i+2]>=76 && imageData.data[i+2]<=172){
                  imageData.data[i]=datas[i];
                  imageData.data[i+1]=datas[i+1];
                  imageData.data[i+2]=datas[i+2];
                }
              }
            }
           }
